...
- **A mechanoid's installed upgrades carry over when it becomes sapient** (if you also run Mechanoid Upgrades, see below), instead of silently vanishing - Big and Small doesn't copy comp state across the conversion by default, so without this fix a mech lost every upgrade the instant it turned sapient, with nothing given back.
...
- **[AV] Mechanoid Skins** - a sapient War Queen's own skin-changer gizmo (see Optional section above) opens and lets you pick a design, but every option in the list currently shows a blank preview instead of the actual design. Not yet fixed, and not yet confirmed whether the applied result itself is affected - off by default until this is sorted out.
...
- **A custom mechanoid head (see "Features" above) can render missing again**, even on a mech it was previously confirmed working on, if the head-rendering fix runs before something else it depends on has finished setting up in that same play session - reloading the save fixes it every time, so this is a one-time-per-session cosmetic glitch, not a lasting break. Root cause (the exact ordering issue) not pinned down yet.
...
- **[AV] Mechanoid Skins** - a sapient War Queen choosing a design for herself now works too (see the Optional section above), alongside choosing a design for her future urchins. Not yet extended to other mechs (Work Queen, Alpha Mechs' War Empress, etc.) - the render tree fix is general-purpose, but each mech needs its own whitelist entry to opt in.
...
- **Let a sapient War Queen use her own [AV] Mechanoid Skins design** (**WIP - DO NOT TOUCH**, off by default) - only does anything if [AV] Mechanoid Skins is also installed. Lets a sapient War Queen use Mechanoid Skins' own skin-changer gizmo on herself, described above. Leaving this off (or turning it off) reverts her to Big and Small's default sapient appearance immediately, even on an existing save, and hides the gizmo too. Known issue: every option in the picker currently shows a blank preview instead of the actual design - see Known Issues.
...
- **Cerebrex Flavour Pack** ships its own compatibility patch for a sapient War Queen's steel reserve/release gizmo (`Compatibility/redmattis.sapientanimals`, auto-loads whenever Big and Small - Sapient Animals is active). No crash risk - it patches different methods than this mod does - but it prefixes `CompMechCarrier.CompGetGizmosExtra` to fully replace vanilla's gizmo generation for any sapient carrier, which skips past this mod's own fix for that specific comp. When both are installed, Cerebrex's implementation decides the War Queen's steel reserve/release gizmo, not this mod's. This mod's "Choose urchin design" feature is unaffected either way, since it hooks `TrySpawnPawns()` itself rather than the gizmo.
